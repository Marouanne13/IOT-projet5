...
@api_view(['GET', 'POST'])
@permission_classes([AllowAny])  # <--- INDISPENSABLE pour l'ESP8266
def measurement_list(request):
    if request.method == 'GET':
        ...
        # Log pour debug : voir ce que l'ESP envoie réellement dans la console Django
        print("Données reçues de l'ESP :", request.data)
        ...
        # Si erreur (ex: mauvais noms de champs), on l'affiche dans la console
...
class MeasurementViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        """
        Autorise tout le monde (AllowAny) pour la création (POST)
        Mais demande d'être connecté (IsAuthenticated) pour le reste (GET, DELETE, etc.)
        """
    ...
    def create(self, request, *args, **kwargs):
        """
        Méthode appelée lors du POST de l'ESP8266
        """
        print("POST reçu dans ViewSet :", request.data) # Debug
        serializer = self.get_serializer(data=request.data)
        
        if not serializer.is_valid():
            print("Détails de l'erreur :", serializer.errors)
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
